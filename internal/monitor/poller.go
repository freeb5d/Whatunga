// Package monitor turns raw RouterOS API replies (maps of string to
// string — that's all the wire protocol gives you) into typed,
// application-friendly structs, and knows how to poll a device for a
// fresh Snapshot.
package monitor
...
// Poller polls one RouterOS device on demand. It owns the underlying
// API connection for the lifetime of the Poller.
type Poller struct {
	device string
	client *routeros.Client
}

// NewPoller connects to a device and returns a Poller ready to take
// snapshots. The caller is responsible for calling Close when done.
func NewPoller(deviceName, address, username, password string, timeout time.Duration) (*Poller, error) {
	client, err := routeros.Dial(address, username, password, timeout)
	if err != nil {
		return nil, err
	}
	return &Poller{device: deviceName, client: client}, nil
}
...
func (p *Poller) systemStatus() (SystemStatus, error) {
...
func (p *Poller) interfaces() ([]Interface, error) {
